...
Даже в тех языках программирования, которые не используют отступы как часть синтаксиса, **все равно используйте их для отделения одних блоков кода от других**. В Python неправильные отступы могут вызвать ошибку, но даже в тех языках, где правильные отступы не являются ошибкой, читать код с отступами все равно приятнее.
...
В python также есть ситуации, когда отступы не будут вызывать ошибку, например, при вызове функции, название и аргументы которой не помещаются в максимальную длину строки и должны быть перенесены на следующую строку.
...
Также, как и при вызове функции.
...
Есть также альтернативное оформление, которое, на наш взгляд, при условии использования отступов выглядит более приятно. Уточните, какой из эти способов принят в команде, к которой вы подключаетесь. Но скобки должны быть.
...
Среди начинающих программистов мы часто встречаем именование переменных либо одним ничего не значащим символом, любо транслитом. 
...
Как первое, так и второе - плохо. Именование с использование транслита не позволит другому разработчику понять, что это за функция и что она делает. Еще одна частая проблема, которая встречается при именовании объектов, - это оставление контекста, который использовался при разработке. Например, мы работаем с датчиком который предоставляет температуру (`temperature`) и давление (`pressure`). Мы хотим как-то обработать данные, например, найти среднее значение. Мы начинаем разработку и выбираем использовать температуру, чтобы проверять нашу работу. Мы правильно пишем название функции, а аргументом передаем список снятых значений температуры, вроде бы все нормально. 
...
Существуют два основных способа именования переменных и функций через подчеркивания (`measure_pressure`) и CamelCase (`measurePressure`). На наш взгляд, через подчеркивание удобнее, так как мы используем название классов через заглавные буквы `PressureSensor` и при беглом чтении разница в одной заглавной (для классов) или строчной (для переменных и функций) менее заметна, чем при использовании подчеркиваний.
При выборе имени для переменной используйте существительные, а для функции глагол.

### Придерживаться определенного подхода

Этот пункт не относится напрямую к стилю кода. И это не является строгим требованием. Код состоит из определенных конструкций, и неплохая идея пользоваться одним подходом для написания одной и той же функциональности. Например, если нам нужно открыть файл, мы можем это сделать несколькими способами:
...
Этот пункт взят из дзена Python и ему можно легко дать количественную оценку - уровни вложенности можно посчитать. Возьмем код ниже: 
...
Хотя мы должны умещать код в 8 строк. Мы можем написать очень насыщенный код. Код ниже заставит разработчика остановиться и потратить какое-то время, чтобы разобраться в нем, если ему нужно что-то поправить или реализовать подобный функционал у себя.
