...
и поставить все нужную библиотеки, используя, например, `requirements.txt`, который часто содержится в репозитории и предназначен для использования:
...
Указав `test: dev`, мы сделали опцию test зависимой от опции dev, и поэтому выполнится сначала она и затем test. 
