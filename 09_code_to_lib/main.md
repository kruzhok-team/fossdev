...
```sh
    ...
    According to sys: 200
```
Мы видим что даже пустой массив занимает какое-то место в памяти (для 64-разрядной системы это 72 байта) и не каждое добавление элементов вызывает выделение памяти. Оговоримся, что в зависимость от того как заполнять данные, результат будет разным. Ниже приведены примеры кода, которые дадут несколько другой вывод, при том же результирующем массиве.
...
``` bash
    ...
    According to sys: 136
 ```   
Мы видим что `sys.getsizeof()` не информативен для вложенных объектов, поэтому оставим только tracemalloc. *Примечание: функции типа `execute_and_get_memory_usage` в python называются декораторы мы не будем подробно на этом останавливаться, так как этот курс не про само программирования и изучения языка. Вы можете легко найти материалы для изучения самостоятельно на подобие [таких](https://www.programiz.com/python-programming/decorator) или [таких](https://www.geeksforgeeks.org/decorators-in-python/)*

Мы получили инструмент для оценки памяти, которая требуется для выполнения той или иной функции и определенным набором входных данных. Теперь мы хотим использовать ее в других наших проектах. Возможно даже она будет интересна другим разработчикам, поэтому нужно оформить все как принято при при распространении кода. Здесь мы пока не будем говорить о менеджерах установки таких как `pip` или `apt` (используется в Ubuntu), поговорим пока о том, что должен быть помимо кода для того, чтобы кодом можно было воспользоваться:
...
Научиться писать хороший README можно только через практику и примеры, которые Вы сами считаете хорошими [здесь](https://github.com/matiassingers/awesome-readme) собран список проектов, которые по мнению автора списка являются хорошими, обратитесь к нему и составьте свое мнение о том что должен включать это файл. 
...
3. Как мы говорили раньше в README должны быть инструкции по установке. Установка может быть полностью ручная, так и с той или иной степенью автоматизации. Например в проектах на Python часто содержится файл `setup.py` в котором прописан код для установки. Тогда инструкция по установке выглядит как одна строка:

    `python setup.py`

4. Любой код использует другой код, поэтому в проекте нужно прописать список используемых библиотек, **зависимостей**. Его можно вести вручную, например в файле `requirements.txt`, который используется `pip` для установки зависимостей, так и автоматически если мы используем другой инструмент например `poetry`.

5. Каталог, который содержит код проекта. Для разных языков программирования он будет называть по-разному. Для проектов на python название каталога совпадает с название проекта. В С++ это будут `src` и `include`. 
...
Напишем файл для установки `setup.py`. Воспользуемся библиотекой `setuptools`.
...
```
Зададим название пакета в поле `name=mtracker`. Поле `install_requires` используется для перечисления списка имен и версий пакетов (точно как в файле requirements.txt). В нашем случае оставим поле пустым, поскольку нашему модулю для работы не требуются сторонние пакеты. Например, если бы вы написали модуль который в процессе работы использует инстурменты `numPy`, `Pandas`, или любой другой модуль не входящий с базовый набор Python, мы бы обяательно указали название и версию этого модуля в поле `install_requires`. Поле `extras_require` содержат модули, которые не нужны при работе приложения, но понадобятся если мы захотим выполнить тесты для проекта, Также следует указать точную версию Python, которая использовалась при сборке модуля. Это необходимо для того, чтоб исключить возможность возникновения ошибок по причине изменения функций или конструкций языка. В которых могла отсутствовать часть методов, модулей и т.д.
...
Мы снабдили написанную нами функцию необходимыми файлами, чтобы ее можно было выложить в открытый доступ и позволить другим разработчикам, устанавливать ее себе и пользоваться ее API. 
