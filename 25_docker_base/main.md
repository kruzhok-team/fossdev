...
What commans use with create in Dockerfile 
The create instruction is not a valid Dockerfile instruction. However, there are a number of other instructions that you can use in a Dockerfile to create and configure your Docker image. Here are some of the most common ones:
•	FROM: Specifies the base image to use for the new image. This is usually the first instruction in a Dockerfile.
•	RUN: Runs a command or series of commands in the container to install dependencies, configure settings, or perform other tasks.
•	COPY or ADD: Copies files from the host system into the container.
...
These instructions can be used in various combinations to create a Docker image that meets your requirements. The exact instructions and configuration will depend on your specific use case and application.
