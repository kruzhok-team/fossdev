...
Dockerfile может быть простым, например состоять из одной сроки: 
...
В сегодняшней части перевода серии материалов о Docker мы поговорим о работе с данными. В частности — о томах Docker. Данные в Docker могут храниться либо временно, либо постоянно. Начнём с временных данных.
...
Создавать тома в Dockerfile можно и используя формат JSON. Кроме того, тома можно создавать средствами командной строки во время работы контейнера. Работа с томами из командной строки. Создать самостоятельный том можно следующей командой:
...
- `daemon` – служба управления объектами Docker: хранилищами, сетями, контейнерами и образами;
- `client` – консольный клиент, который дает возможность взаимодействовать с daemon, отправлять команды, создавать контейнеры и взаимодействовать с ними;
- `image` – неизменяемый образ, из которого будет разворачиваться заданный контейнер;
- `container` – Запущенное приложение, которое развернули из образа (развернутое и запущенное ПО);
- `registry` Служба в Docker, выполняющая функции репозитория (хранилища). Позволяет следить за версиями образов, создавать приватные репозитории. Основным репозитарием является сервис Docker Hub в котором хранятся официальные (проверенные) образы многих компаний разработчиков ПО. Популярный публичный репозиторий, используемый по умолчанию в Docker. Обеспечивает интеграцию с GitHub и BitBucket.
...
1. Гибкость и адаптивность. Благодаря Docker можно легко запускать контейнер как в облачной инфраструктуре, так и на любом локальном устройстве. Можно создавать базовые "шаблоны контейнеров" и использовать их повторно. 
2. Меньше ошибок и противоречий (конфликтов) окружений. В контейнерах Docker содержится всё, что требуется для запуска приложения, поэтому перенос приложений из одной среды в другую не вызывает затруднений. 
3. Скорость развертывания. Так как нет необходимости в настройке рабочего окружения для разработки, тестирования и продакшена, время развертывания проекта на базе контейнерной технологии сокращается в несколько раз. 
4. Универсальность. Docker позволяет использовать любые языки программирования и стек технологий, избавляя от проблемы несовместимости библиотек и технологий.
...
6. Непрерывность работы. С учетом инструментов управления трафиком можно построить процесс обновления приложения таким образом, чтобы обновления одних контейнеров не влияло на работоспособность и стабильность функционирования проекта.
7. Упрощенное администрирование. С помощью Docker легче перенести контейнер с одного хоста на другой, запустить сразу несколько образов, обновить группы контейнеров и откатиться к старой версии.
8. Повышение уровня безопасности. Контейнеры в Docker частично изолированы друг от друга на уровне процессов и ОС, поэтому запуск большого количества контейнеров на одном вычислительном ресурсе исключает риск влияния экземпляра одной ОС на другую, функионирующую в каком-либо в другом контейнере. Сбой в одном контейнере не полияет на работу других изолированных контейнеров.
...
К минусам Docker относится:
1. Высокие требования к ресурсам задействованного устройства. Докеры создают дополнительную логическую прослойку в системе. Это влечет повышенную нагрузку на целевое устройство/сервер/ПК.
2. Необходимость использования оркестратора. Этот момент, связанный с решение задачи балансировки и управления множеством контейнеров. В слчае когда счет активных контейнеров достигает десятков и сотен возникает проблема отслеживания их нагрузкии в режиме реального времени. Например, запуск новых контейнеров для снижения нагрузки с части загруженных серверов или в случае аварийной ситуации в датацентре.   
3. Проблемы при инициализации на MacOS и Windows. Это связано с тем, что изначально «технология» была создана под ОС Linux. Некоторые типы сетей в Docker не поддерживаются.

Тем не менее можно с уверенностью сделать вывод что Docker – это идеальный инструмент для реализации гибкой и надежной микросервисной архитектуры. 
